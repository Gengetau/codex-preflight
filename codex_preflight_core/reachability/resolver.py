...
from codex_preflight_core.repo.collector import SKIP_DIRS
...
class ReachabilityResolver:
    ...
    def _follow_target(
        ...
        if target_path.is_absolute():
            ...
            return
        resolved = self._resolve_candidate(base_dir, target_path, fallback_dir)
        try:
        ...
        if resolved.is_symlink():
            try:
                resolved.resolve().relative_to(self.root)
            except ValueError:
                self.graph.uncertainties.append(uncertainty.outside_repo(target, parent.file))
                return
    ...
    def _resolve_candidate(self, base_dir: Path, target_path: Path, fallback_dir: Path | None) -> Path:
        first = (self.root / base_dir / target_path).resolve()
        if first.is_file() or fallback_dir is None:
            return first
        fallback = (self.root / fallback_dir / target_path).resolve()
