...
@pytest.mark.parametrize(
    ...
    [
        ...
        ('mvn -f "dir/my pom.xml" test', CommandScope.TEST),
        ('mvn --file="dir/my pom.xml" test', CommandScope.TEST),
        ("mvn -s config/settings.xml verify", CommandScope.BUILD),
        ("mvn -P dev test", CommandScope.TEST),
        ("mvn -T 4 test", CommandScope.TEST),
        ("mvn --threads=4 test", CommandScope.TEST),
        ("./mvnw verify", CommandScope.BUILD),
        ("gradle build", CommandScope.BUILD),
        ("gradle --project-dir sub test", CommandScope.TEST),
        ("gradle --project-dir=sub test", CommandScope.TEST),
        ("gradle -p sub -c custom-settings.gradle test", CommandScope.TEST),
        ("gradle --init-script config/init.gradle build", CommandScope.BUILD),
        ('gradle -I "config/my init.gradle" test', CommandScope.TEST),
        ('gradle --init-script="config/my init.gradle" test', CommandScope.TEST),
        ('gradle -c "config/custom settings.gradle" test', CommandScope.TEST),
        ("gradle -i test", CommandScope.TEST),
        ("gradle -I init.gradle test", CommandScope.TEST),
        ("gradle --info test", CommandScope.TEST),
